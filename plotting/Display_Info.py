...
def argparser():
    ...
    parser.add_argument('-info_files', '-f', type=list, dest='info_files', help='List of text files with evtn info, in following order:\n\
                        [IBD_oldPDF_oldClass, alpha-n_oldPDF_oldClass, IBD_oldPDF_newClass, alpha-n_oldPDF_newClass,\
                         IBD_newPDF_oldClass, alpha-n_newPDF_oldClass, IBD_newPDF_newClass, alpha-n_newPDF_newClass]',
                        # default=[
                        ...
                        # ])
                        default=[
                        '/Users/jp643/Documents/Studies/PhD/Antinu/Positronium/Results/2p2labppo/getClass/info_ReactorIBD_0.txt',
                        '/Users/jp643/Documents/Studies/PhD/Antinu/Positronium/Results/2p2labppo/getClass/info_alphaN_13C_0.txt',
                        'nada.txt',
                        'nada.txt',
                        '/Users/jp643/Documents/Studies/PhD/Antinu/Positronium/Results/labppo_6p0_TeDiol_2p5_bismsb_dda_scintillator_directors_review/getPDFs/info_ReactorIBD_0.txt',
                        '/Users/jp643/Documents/Studies/PhD/Antinu/Positronium/Results/labppo_6p0_TeDiol_2p5_bismsb_dda_scintillator_directors_review/getPDFs/info_alphaN_13C_0.txt',
                        '/Users/jp643/Documents/Studies/PhD/Antinu/Positronium/Results/labppo_6p0_TeDiol_2p5_bismsb_dda_scintillator_directors_review/getClass/info_ReactorIBD_0.txt',
                        '/Users/jp643/Documents/Studies/PhD/Antinu/Positronium/Results/labppo_6p0_TeDiol_2p5_bismsb_dda_scintillator_directors_review/getClass/info_alphaN_13C_0.txt',
                        ])
    parser.add_argument('--ratdb_file', '-rf', type=str, dest='ratdb_file', help='ratdb file with old PDFs.',
                        default='/Users/jp643/Documents/Studies/PhD/Antinu/Positronium/Results/'\
                              + 'labppo_6p0_TeDiol_2p5_bismsb_dda_scintillator_directors_review/CLASSIFIER_ALPHAN_IBD_LIKELIHOOD.ratdb')
    parser.add_argument('--index', '-i', type=str, dest='index', help='Index to get PDFs from in ratdb file',
                        default='labppo_6p0_TeDiol_2p5_bismsb_dda_scintillator_directors_review')
    parser.add_argument('--new_file', '-n', type=str, dest='new_file', help='New ratdb file to print PDFs to.',
                        default='/Users/jp643/Documents/Studies/PhD/Antinu/Positronium/Results/'\
                              + 'labppo_6p0_TeDiol_1p5_bismsb_dda_scintillator_directors_review/getPDFs/CLASSIFIER_ALPHAN_IBD_LIKELIHOOD_(new).ratdb')
...
def read_info(in_file):
    ...
    info['recon']['Delta_T'] = []; info['recon']['Nhit'] = []; info['recon']['t_res'] = []
    ...
    for i in range(1, len(lines)):
        ...
        if line[0] == 'MC':
            ...
            info['recon']['Class'].append(float(line[3]))
            if len(line) == 9:
                info['recon']['E'].append(float(line[4]))
                info['recon']['pos'].append(line[5].split(',')); info['recon']['Delta_T'].append(float(line[6]))
                info['recon']['Nhit'].append(float(line[7]))
                info['recon']['t_res'].append(line[8].split(','))
            else: # Added KE
                info['recon']['KE'].append(float(line[4])); info['recon']['E'].append(float(line[5]))
                info['recon']['pos'].append(line[6].split(',')); info['recon']['Delta_T'].append(float(line[7]))
                info['recon']['Nhit'].append(float(line[8]))
                info['recon']['t_res'].append(line[9].split(','))
...
def plotting(fig, canvas, plotting_info, inputs, info, old_info):
    ...
    elif checkboxes['which_plot'].get() == 'Hist':
        plot_alphaN_hist(fig, canvas, plotting_info, inputs, info)
    else:
        print("ERROR: checkboxes['which_plot'] should be 'PDF' or 'hist', not '{}'".format(checkboxes['which_plot']))
    ...
    for process in checkboxes['process']:
        ...
        for plot_name in checkboxes['PDF']:
            ...
            ################ plotting new PDFs ################
                ...
                if plot_name == 'oldPDF':
                    plot_name = process + '_oldPDF_oldClass'
                else:
                    plot_name = process + '_newPDF_oldClass'
...
def plot_alphaN_hist(fig, canvas, plotting_info, inputs, info):
    # Unpack information
    cuts_entries, checkboxes = inputs
    entry_x_min, entry_x_max, entry_Nbins, var_y_log = plotting_info

    # Figure out what to plot
    which_plot = checkboxes['hist']['which_hist'].get().split(' ')
    prompt_delay = which_plot[0]
    plt_quant = which_plot[1]
    ...
    # Set labels
    plot1.set_xlabel(checkboxes['hist']['which_hist'].get())
    plot1.set_ylabel('')
    ...
    for process in checkboxes['process']:
        if not checkboxes['process'][process].get():
            continue
        for plot_name in checkboxes['hist']:
            if plot_name == 'which_hist':
                continue
            if not checkboxes['hist'][plot_name].get():
                continue
            ...
            if prompt_delay not in file_info:
                print('{} is not prompt or delayed!'.format(prompt_delay))
                continue
            if plt_quant not in file_info[prompt_delay]:
                print('{} is not a histogrammable quantity: {}'.format(plt_quant, file_info[prompt_delay].keys()))
                continue
            ...
            if x_min_str != '':
                ...
                x_min = np.min(glob_cut_info[prompt_delay][plt_quant])
            if x_max_str != '':
                x_max = float(x_max_str)
            else:
                x_max = np.max(glob_cut_info[prompt_delay][plt_quant])
            glb_cut_hist_counts, bins = np.histogram(glob_cut_info[prompt_delay][plt_quant], bins = Nbins, range=(x_min, x_max))
            total = np.sum(glb_cut_hist_counts)
        
            ################ plotting the graph with no cuts ################

            if checkboxes['cuts']['noCUTS'].get():
                counts, _bins = np.histogram(file_info[prompt_delay][plt_quant], bins = bins)
                plot1.stairs(counts, bins, color=colours[process][3], linestyle = linestyle,
                             label = label + ', no cuts (N_evts = {})'.format(file_info[prompt_delay]['E'].size))

            ################ plot with general cuts ################

            if checkboxes['cuts']['glbCUTS'].get():
                plot1.stairs(glb_cut_hist_counts, bins, color=colours[process][4], linestyle=linestyle,
                             label = label + ', global cuts, N_evts = {}'.format(glob_cut_info[prompt_delay]['E'].size))
            ...
            for i, Class_info in enumerate(spec_cut_info):
                if checkboxes['cuts']['PDF' + str(i+1)].get():
                    counts, _bins = np.histogram(Class_info[prompt_delay][plt_quant], bins = bins)
                    # plot1.stairs(counts / total, bins, color=colours[process][i], linestyle = linestyle,
                    #             label = label + ', PDF {}, N_evts = {}'.format(i+1, Class_info[prompt_delay]['E'].size))
                    plot1.stairs(counts, bins, color=colours[process][i], linestyle = linestyle,
                                label = label + ', PDF {}, N_evts = {}'.format(i+1, Class_info[prompt_delay]['E'].size))
...
def which_plot_checkboxes(Input_space):
    ...
    var_PDFs_or_hist = tk.StringVar(Input_space, 'PDF')
    ...
    tk.Radiobutton(frame1_title, text='PDFs', variable=var_PDFs_or_hist, value='PDF').pack()
    ...
    var_oldPDF = tk.IntVar(value=0)
    box_oldPDF = tk.Checkbutton(frame1_options, text='old PDFs', variable=var_oldPDF)
    box_oldPDF.grid(row=0, column=1, sticky='nsew')
    var_newPDF = tk.IntVar(value=0)
    box_newPDF = tk.Checkbutton(frame1_options, text='new PDFs', variable=var_newPDF)
    box_newPDF.grid(row=0, column=2, sticky='nsew')
    ...
    frame2_title.rowconfigure(0, minsize=10, weight=1)
    frame2_title.columnconfigure([0, 1], minsize=10, weight=1)

    frame2_radio_button = tk.Frame(frame2_title, width=20)
    frame2_radio_button.grid(row=0, column=0, sticky='nsew')
    tk.Radiobutton(frame2_radio_button, text='Hist', variable=var_PDFs_or_hist, value='Hist').pack()

    frame2_text_input = tk.Frame(frame2_title, width=20)
    frame2_text_input.grid(row=0, column=1, sticky='nsew')
    which_hist = tk.Entry(master=frame2_text_input, width=10)
    which_hist.grid(row=0, column=0, sticky='nsew')
    ...
    var_placeholder = tk.IntVar(value=0)
    box_placeholder = tk.Checkbutton(frame2_options, text='placeholder', variable=var_placeholder)
    box_placeholder.grid(row=0, column=0, sticky='nsew')
    var_newPDF_oldClass = tk.IntVar(value=0)
    box_newPDF_oldClass = tk.Checkbutton(frame2_options, text='new PDF, old classifier', variable=var_newPDF_oldClass)
    box_newPDF_oldClass.grid(row=0, column=1, sticky='nsew')
    var_oldPDF_oldClass = tk.IntVar(value=0)
    box_oldPDF_oldClass = tk.Checkbutton(frame2_options, text='old PDF, old classifier', variable=var_oldPDF_oldClass)
    box_oldPDF_oldClass.grid(row=1, column=0, sticky='nsew')
    var_newPDF_newClass = tk.IntVar(value=0)
    box_newPDF_newClass = tk.Checkbutton(frame2_options, text='new PDF, new classifier', variable=var_newPDF_newClass)
    box_newPDF_newClass.grid(row=1, column=1, sticky='nsew')
    ...
    # Package outputs
    checkboxes = {}
    checkboxes['which_plot'] = var_PDFs_or_hist
    checkboxes['PDF'] = {}
    checkboxes['PDF']['ratdb'] = var_ratdb; checkboxes['PDF']['oldPDF'] = var_oldPDF; checkboxes['PDF']['newPDF'] = var_newPDF
    checkboxes['hist'] = {}
    checkboxes['hist']['which_hist'] = which_hist; checkboxes['hist']['which_hist'].insert(tk.END, 'prompt Class')  # set default
    checkboxes['hist']['placeholder'] = var_placeholder; checkboxes['hist']['newPDF_oldClass'] = var_newPDF_oldClass
    checkboxes['hist']['oldPDF_oldClass'] = var_oldPDF_oldClass; checkboxes['hist']['newPDF_newClass'] = var_newPDF_newClass
    checkboxes['process'] = {}
...
def create_input_section(Input_space):
    ...
    cuts_entries['global']['Both Z'][0].insert(tk.END, '')  # min (m)
...
def main():
    ...
    info_file_names = ['IBD_oldPDF_oldClass', 'alpha-n_oldPDF_oldClass', 'IBD_oldPDF_newClass', 'alpha-n_oldPDF_newClass',
                       'IBD_newPDF_oldClass', 'alpha-n_newPDF_oldClass', 'IBD_newPDF_newClass', 'alpha-n_newPDF_newClass']
    info = {}
